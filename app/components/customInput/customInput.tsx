 ...
 */
const CustomInput = ({ title, placeholder, isDisabled, topic, hasUnit, unit, inSequence }: CustomInputProps) => { 
    ...
    <>
        <div className={`h-full min-h-[32px] p-1.5 sm:p-2 ${inSequence && '!pb-0'}`}>
            <div className='flex flex-row justify-between gap-1 w-full h-full rounded p-1 pt-0.5 pb-0.5 pr-0.5 bg-c-light-smoke shadow-inner'>
                <input title={title} placeholder={placeholder} disabled={isDisabled} value={inputValue} className='flex flex-grow text-black h-full pl-0.5 w-1/3 items-center rounded pt-0.5'/>
